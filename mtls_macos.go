...
// mTLS agent authentication — macOS Agent client (Phase 1 of
// backend/docs/macos-agent-parity-roadmap.md). Ports the Windows agent's
// mtls_windows.go design 1:1: local keypair generation (the private key
// never leaves the device), CSR-based one-time registration via a Managed-
// Configuration-delivered Global Bootstrap Token, and a self-sustaining
// renewal loop that never needs a bootstrap token again after first
// enrollment. See the roadmap doc's §1 for why this file's design choices
// (file-based keystore, deferred Keychain integration) were made
// deliberately rather than assumed.
//
// Keystore is file-based, matching the Windows agent's own disclosed v1
// simplification rather than the "correct" platform answer (Keychain/
// SecItem here, Certificate Store/CNG there) — this agent already runs as
// root via LaunchDaemon for every other privileged operation it performs
// (fdesetup, socketfilterfw, sysadminctl), so root-owned 0600 files under a
// root-owned 0700 directory ARE the actual security boundary here, with no
// separate ACL tool needed the way Windows' icacls dance was (Unix
// permissions already express "root only" natively — there's no equivalent
// gap to close).
//
...
func mtlsDir() string {
	return "/Library/Application Support/Applivery/SOAR/mtls"
}
...
// backend (sendWebhook, fetchCustomChecks, and future callers) goes through
// this single function, so there is exactly one place that decides which
// auth mode is active — no call site can forget to check.
...
// header is simply omitted — a device is either on legacy shared-secret
// auth or on mTLS, never sending both at once.
...
// cycle (telemetry_macos.go) — no separate ticker/goroutine needed, since
...
func ensureMtlsIdentity() {
	...
	if id == nil {
		if config.BootstrapToken != "" {
			...
			// (see Config.RegisterURL's doc comment in config.go), so it
			// doesn't need BaseURL's dedicated mTLS-configured domain at all.
			// Falls back to baseURL when RegisterURL is empty (single-URL
			// deployments).
...
// rather than a fixed day count that would either be too aggressive for a
// short-lived cert or dangerously late for one just above the configured
// floor. Identical logic to the Windows agent's own withinRenewalWindow.
...
// registerMtlsIdentity — POST /api/device-mtls/register. Deliberately uses a
// PLAIN http.Client (no client cert — this device doesn't have one yet) and
// authenticates via the one-time X-Bootstrap-Token header instead. The
// backend forces the issued certificate's CN to match the device's own
// serial number regardless of what CN this CSR claims, so there's no
// security reliance here on this agent asking for the "right" identity,
// only on it asking with the right token.
...
// renewMtlsIdentity — POST /api/device-mtls/renew. The self-sustaining loop:
// the request authenticates via the device's CURRENT, still-valid
// certificate (mtlsHTTPClient below picks it up automatically), never a
// bootstrap token, and a fresh keypair+CSR is generated for the
// replacement — the private key is never reused across renewals either.
...
// in-memory identity used by mtlsHTTPClient AND the on-disk keystore used to
// survive a restart — both updated together so the two never disagree about
// which certificate is current.
...
// is loaded yet), so a daemon restart doesn't force an unnecessary
...
// writeKeystoreFileAtomic creates the keystore directory root-only (0700)
// and writes the file root-only (0600) via a temp-file-then-rename, same
// atomicity guarantee as the Windows agent's own keystore writer. Since this
// process always runs as root (LaunchDaemon), these plain Unix permissions
// already are the access-control boundary — no separate ACL tool call is
// needed the way Windows' icacls invocation was; only root can read a
// root-owned 0600 file on macOS, full stop.
